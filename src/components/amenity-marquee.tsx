...
export function AmenityMarquee() {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-cream py-3 sm:py-4 border-y border-sand/30 shadow-xs">
      ...
      {/* Ticker Row with Smooth Hover-Pause */}
      <div className="flex w-max animate-marquee hover:[animation-play-state:paused] space-x-4 sm:space-x-6 px-4">
        {marqueeItems.map((item, idx) => {
          const Icon = item.icon
          return (
            <div
              key={`${item.label}-${idx}`}
              className="flex items-center space-x-2.5 sm:space-x-3 px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-full bg-ink/90 backdrop-blur-xl border border-sand/30 hover:border-gold/60 shadow-warm-sm text-cream text-xs sm:text-sm font-medium transition-all duration-300 group cursor-default shrink-0"
            >
