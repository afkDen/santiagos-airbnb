      ...
      <div className="site-container grid min-h-[calc(100dvh-72px)] grid-cols-1 items-center gap-8 py-8 lg:grid-cols-12 lg:gap-12 lg:py-10">
        <div className="order-1 space-y-6 lg:col-span-5">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <Link
              href="/rates"
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-sand/35 bg-cream/10 px-6 py-3 text-sm font-bold text-cream transition-[background-color,border-color,transform] duration-200 hover:border-sand/60 hover:bg-cream/15"
            >
              ...
              <div className="grid grid-cols-4 gap-2 rounded-2xl bg-ink/[0.82] p-2 backdrop-blur-md sm:gap-3 sm:p-3">
                {HERO_VIEWS.map((view, index) => (
                  <button
                    ...
                    className={`relative min-h-11 overflow-hidden rounded-xl px-2 py-2 text-[11px] font-bold transition-[color,transform] duration-200 sm:text-xs ${
                      activeView === index
                        ? 'text-ink'
